...
event1 <- evidf %>%
  ...
  mutate(myperiod = ifelse(year %in% pre1, 'pre1', 
                           ifelse(year %in% dr1, 'dr1',
                           ifelse(year %in% post1, 'post1', 'out')))) %>%
  filter(myperiod != 'out') 
...
evidf %>%
  ...
  mutate(event1 = ifelse(year %in% pre1, 'pre1',
...
http://www.ideandalucia.es:80/services/dea100/wfs?request=GetFeature
