...
r <- r %>%
  select(year = AGNO, month = MES, pre=PREFINAL, tmed = TEMPMED) %>%
...
rba_s <- rba %>% filter(!(hyear_f %in% c('2004-2005', '2011-2012'))) 
  ggplot(rba, aes(x=as.factor(hmonth), y=csum)) + 
  geom_boxplot() +  
  geom_smooth(aes(group=1), se=TRUE, level=0.95, colour='black') + 
  geom_line(data=subset(rba, hyear_f == '2004-2005'),
            aes(x=as.factor(hmonth), y=csum, group=1), colour='blue') +
...
ltdf <- ltdf %>% 
  select(cod = INDICATIVO, VARIABLE, FECHA, VALOR) %>% 
...
# We used onlyt the 5514 station (the most long term)
...
st5514 <-ltdf %>% 
  ...
  filter(cod == '5514') %>% 
  filter(hyear >= 1940) %>% 
  select(fecha, month, hmonth, hyear, hyear_f, pre=VALOR)
