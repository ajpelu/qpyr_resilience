...
# Read data directly from repo modis_iv
rawdata <- read.csv(text = getURL("https://raw.githubusercontent.com/ajpelu/modis_iv/master/data/iv_quercus_pyrenaica.csv"), header=T) 

# There are some duplicates records. Remove them 
rawdata <- unique(rawdata)


# Read and prepare data (from Linaria)
# rd <- read.csv(file=paste(di, "/data_raw/evi/raw_iv.csv", sep= ""), header = TRUE, sep = ',')
```
...
# Which pixels has less than 23 images per year: 
less23 <- rawdata %>% 
  mutate(year = lubridate::year(date)) %>% 
  group_by(year, iv_malla_modi_id) %>% count() %>% 
  dplyr::filter(n < 23) 

less23 %>% group_by(year) %>% count()
```

Parace que en 2000, 2013 y 2015 hay menos de 23 imagenes por año 

```{r}
## Anomalies by pixel
rawdata %>% 
  filter(iv_malla_modi_id == 142799) 


%>% 
  ggplot(aes(x=composite_day_of_the_year, y=evi)) + geom_point()
```
...
16*3
         ...
         iseq = ifelse(yearday == composite_day_of_the_year, 1, 0))
...
myid <- ndvi_by_date %>% filter(iv_malla_modi_id == 142799) 
