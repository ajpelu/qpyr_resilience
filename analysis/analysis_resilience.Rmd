...
# Model 
myformula <- as.formula(paste0(variable, " ~ elev + clu_pop + elev:clu_pop"))
...
## Effects 
# Population 
plot(allEffects("elev:clu_pop",mymodel)) 
...
df_letter <- fortify(cld(tuk)) %>%
  transmute(clu_pop = as.factor(lhs),
         tukey_clu_pop = letters) %>%
  mutate(variable = variable)


effect("neuroticism*extraversion", mod.cowles)


plot(effect("clu_pop", mymodel))


     x.var='Q. pyrenaica cluster'))
plot(effect("clu_pop", mymodel), style='lines')

mye <- allEffects(mymodel)
mye
as.data.frame(mye[[1]])
...
# Para ver el plot de los efectos 
aa <- effect("clu_pop", mymodel)
plot(allEffects("elev:clu_pop",mymodel))

plot(aa, x.var="clu_pop")



# Interplot 
lm_mymodel <- lm(myformula, data=eviresi_f)
interplot(m=lm_mymodel, var1="elev", var2 = "clu_pop")
...
# 
myvar <- c('elev','clu_pop')
cc <- corvif(eviresi_f[, myvar])
