...
Loop to create raster map
  ...
  for (i in resilience_indicators) { 
    ...
    # raster_layer 
    r_resi <- rasterize(aux_spatial, aux_rast, i, fun=mean)
    names(r_resi) <- paste0(i,'_', event, '_', season)
    
    # reprojected raster
    r_resi_re <- projectRaster(r_resi, crs=crs("+init=epsg:4326"))
    names(r_resi_re) <- paste0(i,'_', event, '_', season)
    
    # assign 
    name_raster <- paste0("r_",i,'_', event, '_', season)
    name_raster_re <- paste("r_",i,'_', event, '_', season, "_re")
    
    out <- writeStart(r_resi, name_raster)
    out_re <- writeStart(r_resi_re, name_raster_re)
  ...
  out <- writeStop(out)
  out_re <- writeStop(out_re)
  return(out)
  return(out_re) 
} 


createMapsResi(eviresi1_summer, event=1, season='summer')



df_sel <- eviresi1_summer
event <- 1
season <- 'summer' 
