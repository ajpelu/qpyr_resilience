...
# Annual 
auxilio <- eviresi %>% filter(seasonF == 'annual') 
...
for (j in c(1:2)){ 
  auxilio <- auxilio %>% filter(event==j) 
  ...
  auxdf <- auxdf %>% mutate(event = j)  
  
  auxaux <- rbind(auxaux, auxdf)} 
...
gpop_bar
pdf(file=paste0(di, "/images/plot_resicomp_bar_by_cluster_annual_event1.pdf"), height = 7, width = 8)
gpop
dev.off()
