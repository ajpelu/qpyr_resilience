...
resilience_indicators <- c('rs','rt', 'rc')
...
# Create stack of 
stack_resi <- stack(r_rc, r_rs, r_rt) 

# Export stack 
temp <- getwd()
setwd(paste(di, "/data/raster/", sep=""))
writeRaster(stack_resi, filename = 'r_tau_stack', overwrite =TRUE) 
setwd(temp)
...
aux_rast <- raster(eviresi_sp, resolution=250)

r_rs <- rasterize(eviresi_sp, aux_rast, 'rs', fun=mean)
names(r_rs) <- 'rs'
...
levelplot(r_rs, 
          ...
          pretty=TRUE,
          contour=TRUE,  
          at=seq(minValue(r_rs), maxValue(r_rs), by=.025))
...
# Define extension 
extent(aux_r) <- extent(eviresi_sp)

# Rasterize mq_evi_mean
r_rs <- rasterize(eviresi_sp, aux_r, res=c(250,250), 'rs')
